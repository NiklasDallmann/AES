...
public:
	...
	void encrypt(const uint8_t *plainBlock, uint8_t *cipherBlock)
	{
		...
		DEBUG("PRECOMPUTED GF(2^8) MUL3")
		std::stringstream xtimes;
		
		xtimes << "0x" << std::setw(2) << std::setfill('0') << std::hex << 0;
		xtimes << ", ";
		
		for (uint8_t i = 1; i < 0xff; i++)
		{
			xtimes << "0x" << std::setw(2) << std::setfill('0') << std::hex << uint32_t(this->_xtime(i) ^ i);
			xtimes << ", ";
		}
		
		xtimes << "0x" << std::setw(2) << std::setfill('0') << std::hex << uint32_t(this->_xtime(0xff) ^ 0xff);
...
//			this->_state[column][3] = _xtime(word[0]) ^ word[0] ^ word[1] ^ word[2] ^ _xtime(word[3]);
			...
			this->_state[column][0] = _galoisMultiply_2[word[0]] ^ _galoisMultiply_2[word[1]] ^ word[1] ^ word[2] ^ word[3];
			this->_state[column][1] = word[0] ^ _galoisMultiply_2[word[1]] ^ _galoisMultiply_2[word[2]] ^ word[2] ^ word[3];
			this->_state[column][2] = word[0] ^ word[1] ^ _galoisMultiply_2[word[2]] ^ _galoisMultiply_2[word[3]] ^ word[3];
			this->_state[column][3] = _galoisMultiply_2[word[0]] ^ word[0] ^ word[1] ^ word[2] ^ _galoisMultiply_2[word[3]];
		}
	}
